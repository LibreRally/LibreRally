...
public class CodeFirstSceneFactoryTests
{
    ...
    public void CreateMainScene_CreatesExpectedRootEntities()
    {
        ...
        var ground = Assert.Single(scene.Entities.Where(entity => entity.Name == "Ground"));
        Assert.NotNull(ground.Get<VehicleSpawner>());

        var camera = Assert.Single(scene.Entities.Where(entity => entity.Name == "Camera"));
        Assert.NotNull(camera.Get<CameraComponent>());
        Assert.NotNull(camera.Get<BasicCameraController>());

        var directionalLight = Assert.Single(scene.Entities.Where(entity => entity.Name == "Directional light"));
